...
11. Run `reviewworthy remote plan`. For a Pull Request, it recomputes the complete Diff and requires exact agreement across `base_sha`, `head_sha`, `patch_sha256`, `changed_files`, `additions`, and `deletions`; the Packet repository identity and live Issue identity must also match, and a PR Body must contain the canonical supporting Issue URL. Only after the user confirms the displayed operation ID and all signal/policy gates pass may the CLI create the requested Issue or formal Pull Request through `gh`. For an Issue-backed PR, the CLI then searches for the exact PR URL note, writes at most one such note, and records `needs_reconciliation` if the note cannot be written; it never creates a second PR.

...
Remote readiness also requires an approved Contribution Contract, a confirmed Contribution Signal for Discovery or signal-backed work, passed Orientation and Assessment, verification commands/evidence, and deterministic scope/budget checks. Reproducible Discovery evidence additionally needs an explicit policy allowance. If policy requires a Draft PR, the operation includes that state and invokes `gh pr create --draft`.

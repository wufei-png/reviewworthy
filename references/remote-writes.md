...
Remote readiness also requires an approved Contribution Contract whose approval snapshot still matches the Contract, a Packet `0.2` merge-base PR Diff receipt (`base_tip_sha`, `merge_base_sha`, `head_sha`, patch hash, changed files, and counts), a CLI-executed verification receipt with exit code 0 and matching `head_sha`, a valid non-rejected Contribution Signal for Discovery or signal-backed work, a recorded successful public-reference verification for Issue-backed work, passed Orientation and Assessment, and deterministic scope/budget checks. Maintainer confirmation is optional. Reproducible Discovery evidence additionally needs an explicit policy allowance. If policy requires a Draft PR, the operation includes that state and invokes `gh pr create --draft`. `stdout` and `stderr` hashes are audit fields only; they do not establish test coverage.

For a Pull Request backed by an Issue, the final Body must contain the canonical Issue URL. After the PR exists, Reviewworthy uses one receipt with the lifecycle `pending → pr_created → link_attempted → linked` or `needs_reconciliation`. It first searches the Issue comments for an exact one-line PR URL. Only when that note is absent does it check that the Issue is present, unlocked, and commentable, then write that one note. A failed note write records `needs_reconciliation`; it never creates another PR and the CLI does not retry the note automatically.
